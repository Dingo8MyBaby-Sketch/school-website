					...
					<div class="col-lg-4 col-md-6 col-sm-6 col-xs-12 mb-4">
						<div class="card h-100">
							<?php if(!empty($s->photo)): ?>
								<img src="<?= base_url('img/staff/' . $s->photo) ?>" class="card-img-top" alt="<?= $s->name ?>" style="height: 300px; object-fit: cover;">
							<?php else: ?>
								<img src="<?= base_url('img/identitas/logo.png') ?>" class="card-img-top" alt="<?= $s->name ?>" style="height: 300px; object-fit: cover;">
							<?php endif; ?>
							<div class="card-body">
								<h5 class="card-title"><?= $s->name ?></h5>
								<p class="card-text"><strong><?= $s->position ?></strong></p>
								<?php if(!empty($s->email)): ?>
									<p class="card-text"><small><i class="fas fa-envelope"></i> <?= $s->email ?></small></p>
								<?php endif; ?>
